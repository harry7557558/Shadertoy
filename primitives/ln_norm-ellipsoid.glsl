...
// Estimate the SDF to (x/rx)^n + (y/ry)^n + (z/rz)^n = 1
...
float equation(vec3 p, vec3 r, float n) {
    p = abs(p);
    return pow(p.x/r.x,n)+pow(p.y/r.y,n)+pow(p.z/r.z,n)-1.0;
}

// results in too small gradient
float sdf_naive(vec3 p, vec3 r, float n) {
    float d = pow(dot(pow(abs(p)/r,vec3(n)),vec3(1)),1./n)-1.0;  // definition
    float m = min(min(r.x,r.y),r.z);  // guarantee length(grad)<=1
    return d * m;
}

// divide by gradient
// gradient equals 1/n for p->∞, too small for high degree
// possible division by zero when inside the shape
float sdf_divbygrad(vec3 p, vec3 r, float n) {
    p = abs(p);
    float v = dot(pow(p/r,vec3(n)),vec3(1))-1.0;
    vec3 grad = n/pow(r,vec3(n)) * pow(p, vec3(n-1.));
    return v / length(grad);
}

// inspired by iq's ellipsoid sdf
// possible division by zero when inside the shape
float sdf_rearrange(vec3 p, vec3 r, float n) {
    p = abs(p);
...
float sdf_inout(vec3 p, vec3 r, float n) {
    p = abs(p);
    float k1 = pow(dot(pow(p/r,vec3(n)),vec3(1)),1.0/n);
    float k2 = pow(dot(pow(p/(r*r),vec3(n)),vec3(1))+0.0001,1.0/n);
    return k1*(k1-1.0)/k2;
}

// max norm distance
// gradient has a C0 continuity and may be smaller than 1
float sdf_maxnorm_ellipsoid(vec3 p, vec3 r) {
    p = abs(p);
    float a = dot(vec3(1)/(r*r), vec3(1.0));
    float b = dot(p/(r*r), vec3(1.0));
    float c = dot((p*p)/(r*r), vec3(1.0))-1.0;
    if (c>0.0) return sdf_inout(p, r, 2.0);
    float d = (-sqrt(b*b-a*c)+b)/a;
    return d;
}

float sdf(in vec3 p) {
    vec3 r = 10.0*vec3(2.0,1.0,0.5);
    p.y = max(p.y, 0.);
    float d = sdf_inout(10.0*p, r, 2.0)/10.0;
    d = sdf_maxnorm_ellipsoid(10.0*p, r)/10.0;
    //d += 0.1;
    //d = abs(d);
    return d;
}
...
vec3 render(in vec3 ro, in vec3 rd, float t0, float t1) {
    ...
    grad = normalize(grad);
    col = 0.2+0.05*grad.y+col*max(dot(grad, light),0.0);
