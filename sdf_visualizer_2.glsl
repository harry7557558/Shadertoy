...
float sdf(in vec3 p) {
    if (p.z<-1.0) return -p.z;
    float r = 1.0 + 0.01*sin(32.0*atan(length(p.xy-vec2(1,0)), p.z));
    float d1 = length(p-vec3(1,0,0)) - r;
    float b = 0.8 + 0.02*sin(10.0*p.x)*sin(10.0*p.y)*sin(10.0*p.z);
    float d2 = max(max(abs(p.x+1.0), abs(p.y)), abs(p.z)) - b;
