...
vec3 sampleHenyeyGreenstein(float g, vec3 wi, inout vec3 m_col) {
    vec3 wo = sampleUniformSphere();
    float u = dot(wi, wo);
    float p = (1.0-g*g)/pow(1.0+g*g-2.0*g*u,1.5) / (4.0*PI);
    m_col *= p * (4.0*PI);
    return wo;
}

vec3 importanceSampleHenyeyGreenstein(float g, vec3 wi, inout vec3 m_col) {
...
void calcScatter(inout float t, inout vec3 rd, inout vec3 m_col, out int material, inout vec3 min_n) {
    ...
    const vec3 absorb = exp(-0.05*vec3(0.8, 0.6, 0.05));  // absorb per hit
    ...
    #if 1
        if (right) rd = importanceSampleHenyeyGreenstein(0.2, rd, m_col);
        else rd = sampleHenyeyGreenstein(0.2, rd, m_col);
    #else
        if (right) rd = importanceSampleHenyeyGreenstein(0.7, rd, m_col);
        else rd = importanceSampleHenyeyGreenstein(-0.7, rd, m_col);
    #endif
        m_col *= absorb;
        material = mat_none;
    }
