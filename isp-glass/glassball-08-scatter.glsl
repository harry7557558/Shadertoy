...
void calcAbsorb(in vec3 p, out vec3 sabs, out float k) {
    p -= vec3(0, 0, 1);  // center of sphere
    sabs = vec3(1.0), k = 1e-3;
    float d = sqrt((p.x*p.x)/(0.2*0.2)+(p.y*p.y)/(0.4*0.4)+(p.z*p.z)/(0.2*0.2))-1.0;
    if (d < 0.) {
        k = 40.0;
        sabs = sin(30.0*p.y)+0.5*sin(10.0*p.y)<0. || d<-0.2 ? vec3(0.99, 0.95, 0.2) : vec3(0.5,0.2,0.2);
    }
    d = p.z + 0.4;
    if (d < 0.) {
        k = 5.0;
        sabs = vec3(0.8, 0.98, 0.85);
    }
}
...
void calcScatter(in vec3 ro, inout vec3 rd,
    ...
    float dt = 0.01;  // smaller is slower but better
    for (float t = 1e-4; t < mt-dt; t += dt) {
        vec3 sabs; float k;
        calcAbsorb(ro + rd * (t+0.5*dt), sabs, k);
        float dp = exp(-k*dt);
        if (p * dp < hit_p) {
            mt = t + dt * log(p/hit_p)/k;
            rd = sampleHenyeyGreenstein(0.99, rd);
            m_col *= sabs;
            material = mat_none;
            min_n = vec3(0.0);
            return;
