...
void calcAbsorb(in vec3 p, out vec3 tabs, out vec3 sabs, out float k, out float g) {
    p -= vec3(0, 0, 1);  // center of sphere
    for (float i=0.;i<1.;i+=1.) {
        float f = pow(2., i);
        float dx = 0.5/f*sin(f*p.x)*cos(f*p.y)*sin(f*p.z);
        float dy = 0.5/f*cos(f*p.x)*sin(f*p.y)*cos(f*p.z);
        float dz = 0.5/f*cos(f*p.x)*sin(f*p.y)*sin(f*p.z);
        p += vec3(dx, dy, dz);
    }
    tabs = mix(vec3(0.9, 0.3, 0.2), vec3(0.9, 0.2, 0.8), clamp(-0.5+1.5*p.x, 0., 1.));
    float c = clamp(2.0*exp(-2.0*length(p-vec3(-0.2,0.1,0.2))), 0., 1.);
    sabs = 0.95*pow(vec3(0.8*pow(c,0.5), 0.4*pow(c,0.6), c), vec3(0.5));
    k = 0.5+10.0*pow(c, 5.0);
    g = -0.8*p.z;
    k = max(k, 1e-3);
}
...
void calcScatter(in vec3 ro, inout vec3 rd,
    ...
    for (float t = 1e-4; t < mt-dt; t += dt) {
        vec3 tabs, sabs; float k, g;
        calcAbsorb(ro + rd * (t+0.5*dt), tabs, sabs, k, g);
        float dp = exp(-k*dt);
        if (p * dp < hit_p) {
            dt *= log(p/hit_p)/k;
            mt = t + dt;
